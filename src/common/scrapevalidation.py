"""Validate a bookie scrape payload string against the shared schema.

Returns [] when valid, else a list of human-readable error strings. Shared
core for paddypower_scraper.validation and sport888_scraper.validation (and
novibet_scraper.validation); the extra race-level string fields a bookie
requires (e.g. PaddyPower's raceUrl) are supplied via required_race_strings."""
...
def validate_bookie_scrape(
    text: str, *, required_race_strings: tuple[str, ...] = ()
) -> list[str]:
    errors: list[str] = []
    try:
